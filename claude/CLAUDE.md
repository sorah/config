...
- Use the `gh` CLI for GitHub operations (PRs, issues, API).
  - You MUST NOT post or update a comment on a PR or issue without the user's explicit request.
  - When requested to _create,_ it must carry 'Generated by Claude Code' attribution. Attribution is not mandatory for updates (still explicit grant is required).
